...
pub async fn run() -> anyhow::Result<()> {
    ...
    match state.write_consul(endpoint, key).await {
        ...
        Err(e) => terminal::warn(&format!(
            "write_consul failed (expected without Consul): {}",
            e
        )),
    }
    ...
    match State::read_consul(endpoint, key).await {
        Ok(read_state) => {
            ...
            terminal::info(&format!(
                "Read {} resources from Consul",
                read_state.resources.len()
            ));
        }
        Err(e) => terminal::warn(&format!(
            "read_consul failed (expected without Consul): {}",
            e
        )),
    }
