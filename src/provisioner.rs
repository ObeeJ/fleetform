use crate::{config::Config, hcl::parse_blocks, state::State, terminal};
...
impl Provisioner {
    ...
    async fn describe_tagged_instances(
        &self,
        environment: &str,
        ...
        for block in blocks {
            ...
            if block.labels.len() != 2 {
                return Err(anyhow::anyhow!(
                    "Invalid resource block: expected resource type and name"
                ));
            }
            ...
            state
                .resources
                .push(format!("{}:{}", resource_type, resource_name));
        }
